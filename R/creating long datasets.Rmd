---
title: "LCA with pooled observations"
...
# Create new dataset that includes biofeedback to be merged before part 2.
wave.1 <- a.cams.lta
################################################################################
# Wave 2 
...
# Create dataset that includes biofeedback to be included in merged long dataset in part 2
wave.2 <- b.cams.lta
################################################################################
# Wave 3 
...
# Create dataset that includes biofeedback to be included in merged data in part 2
wave.3 <- c.cams.lta
...
```
```{r, echo = FALSE}
# merge wave1 wave2 wave 3 
# Merge with Wave 2 
w1w2 <- merge(wave.1, wave.2, by = "M2ID")
# Merge with Wave 3 
df.with.bio <- merge(w1w2, wave.3, by = "M2ID")

# turn into long dataset
df.long.bio <- long_panel(df.with.bio, prefix = "_W", begin = 1, end = 3, label_location = "end")
# save dataset
save(df.long.bio, file = "dflongbio.rda")

# Save cam.lta.2 as dataset
save(cams.lta.2, file = "camslta2.rda")

save(cams.long.df, file = "camslongdf.rda")
```
