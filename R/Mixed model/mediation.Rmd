...
This chapter examines the relationships between SES, CAM use, and health. An examination of the role of AHL as a mediator of the socioeconomic status-health relationship represents an important contribution to health lifestyles research. The association between social position and health (i.e. the social gradient in health) has been well-documented, and health lifestyle represents an important pathway by which socioeconomic status impacts health (Adler et al. 1994; Brunello et al. 2016; Burdette and Hill 2008; Lantz et al. 1998, 2001). Particularly in public health, the appeal of investigating health lifestyles as a pathway by which socioeconomic status impacts health has been the possibility of addressing health disparities via campaigns aimed at altering individual behaviors. This approach has typically relegated health lifestyles to the position of individual behavioral attributes, ignoring the social context within which behaviors are enacted (Frohlich, Corin, and Potvin 2001). Health lifestyle theory emphasizes that behaviors are not simply the product of individual motivations or will but reflect the enabling and constraining aspects of structural position (Abel and Frohlich 2012; Cockerham 2005; Frohlich et al. 2001). Thus, I expect to find that individuals sharing a similar socioeconomic position will also share similar health lifestyles, reflecting the important role of life circumstances and social experience in shaping health lifestyles.

Analysis is conducted in two parts. In the first step of analysis I address the question of whether CAM use is related to SES as measured by income and education. Outcomes include a measure of total CAMs used in the past year as well as binary indicators of whether respondents use each of the types of CAMs identified in the previous chapter (mind-body medicines, alternative medicines, physical and nutritional approaches, and manipulative treatments). I hypothesize that CAM use is positively related to income (*Hypothesis 1a*) and education (*Hypothesis 1b*). 

In the second step of analysis I examine two possible mediation processes. Path models will provide a longitudinal test of *Hypothesis 1*, as well as test the relationship between SES and health. As has been observed in previous research, I expect to find higher SES to be related to better health outcomes (*Hypothesis 2*). The first mediation process I will examine is whether CAM use mediates the relationship between SES and health. I expect that individuals with high SES are more likely to use CAMs, and that this in turn leads to better health outcomes (*Hypothesis 3*). The second mediation process I examine is whether health mediates the relationship between SES and CAM use. While I expect that CAM use influences health, it is also possible that health influences CAM use. Thus it is important to examine both directions of this relationship. By examining both directions of this possible relationship, I will be able to tease out whether CAMs tend to be used for prevention or treatment of disease. I expect to find that SES influences health, and that health in turn influences CAM use (*Hypothesis 4*).
...
The research question examined in this chapter is whether socioeconomic status (SES) predict the use of mind body approaches, alternative medical systems, physical and nutritional approaches, manipulative treatments, and total CAMs. To address this question. I used generalized linear mixed models (GLMMs) to assess these relationships. I used logistic GLMMs to model the effect of SES on each of the CAM factor indices and a Poisson GLMM to model the effect of SES on total CAM use. 

GLMMs are an extension of generalized linear models that account for the nested nature of data. In the MIDUS data, each respondent participated in the survey at one to three time points. Therefore measures at different time points are nested within the individual. GLMMs include fixed effects for level 1 predictors and random effects for level 2. The fixed effects in the models below include all predictors and covariates. Individual id numbers were used to obtain the random effects. The intercept for each individual can vary randomly. Analyses are completed in R using the 

Mediation analyses are conducted using structural equation modeling (SEM). In cases such as the present study where all variables are observed as opposed to unobserved (i.e. latent) it is also known as path analysis. SEM is an extension of multiple regression. It allows for the inclusion of more than one dependent variable in a model, and allows for the testing of models that specify chains of causality. The SEM models in this chapter simultaneously estimate two mediation processes. First, CAM use as a mediator of the relationship between SES and health. Second, health as a mediator of the relationship between SES and CAM use. SES is measured by income and education at wave 1. CAM use and health are measured at waves 2 and 3, with the wave 2 variables representing mediators and the wave 3 variables being the outcome of interest. Wave 3 variables are lagged, meaning the wave 2 variable is included as a control. This allows for isolating the effect of the mediator independent of the effect of the outcome variable at the previous wave. 

An important aspect of SEM is model fit. Fit refers to how well the relationships in the model reproduce the relationships observed in the data. Goodness of fit indices used in the following analyses to assess model fit include \$\\chi\$^2^ tests, comparative fit index (CFI), Tucker Lewis Index (TLI), root mean square error of approximation (RMSEA), and standardized root mean square residual (SRMR). A non-significant \$\\chi\$^2^ indicates good model fit, though the test is effected by sample size, with larger sample sizes almost always producing a significant result. Cutoffs indicating good fit include CFI > 0.90, TLI > 0.90, RMSEA < 0.08, and SRMR < 0.08. 

Analyses were first completed in R version 4.2.0 using the lavaan program; however, the outcome variables in the models are a mixture or binary, count, and ordinal outcomes. Lavaan does not allow the user to specify non-normal distributions for endogenous (i.e. outcome) variables, assuming all variables are normally distributed. To address the violated assumption, analyses were also completed in Stata Version 17 using the gsem command for generalized SEM (GSEM). The gsem command allows the user to specify non-normal distributions such as logit or poisson. In GSEM analyses below, total CAMs is estimated with a Poisson distribution, binary outcomes are estimated with a binomial distribution and logit link, and SRH, an ordinal outcome, is estimated with an ordinal distribution and ordered probit link function. Goodness of fit indices presented below are from SEM analyses in lavaan. Goodness of fit indices are currently not available in gsem. Both SEM analyses using lavaan and GSEM analyses using gsem are presented below.  
