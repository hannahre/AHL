...
(SEM - include appropriate fit indices cut offs and what each fit index tests). 
...
# Run model and get summary and fit statistics
set.seed(1234)
model.summ <- sem(model, data = path.complete, se = "boostrap")
summary(model.summ, standardized = TRUE, fit.measures = TRUE)
fitMeasures(model.summ, c("cfi", "rmsea", "srmr"))
#modificationindices(model.summ)
...
parameterEstimates(model.summ, boot.ci.type = "bca.simple", standardized=TRUE) %>% 
  ...
  knitr::kable(digits = 3, 
               booktabs=TRUE, 
               caption="Direct Effects: SES, Total CAMs, and Depression",
               escape = F, 
               col.names = c("Outcome", "Indicator", "\\textit{b}", "Lower CI", 
                             "Upper CI", "\\textit{p}-value", " ", "$\\beta$")) %>%
  kable_classic(html_font = "CMU Serif") %>%
...
iemindbodydepression.table %>% 
  mutate(Path = replace(Path, Path == "_nl_1", "Indirect Effect: Income \U2192 Depression \U2192 Mind-Body Medicine")) %>%
  mutate(Path = replace(Path, Path == "_nl_2", "Total Effect: Income \U2192 Depression \U2192 Mind-Body Medicine")) %>%
  mutate(Path = replace(Path, Path == "_nl_3", "Indirect Effect: Income \U2192 Mind-Body Medicine \U2192 Depression")) %>%
  mutate(Path = replace(Path, Path == "_nl_4", "Total Effect: Income \U2192 Mind-Body Medicine \U2192 Depression")) %>%
  mutate(Path = replace(Path, Path == "_nl_5", "Indirect Effect: Education \U2192 Depression \U2192 Mind-Body Medicine")) %>%
  mutate(Path = replace(Path, Path == "_nl_6", "Total Effect: Education \U2192 Depression \U2192 Mind-Body Medicine")) %>%
  mutate(Path = replace(Path, Path == "_nl_7", "Indirect Effect: Education \U2192 Mind-Body Medicine \U2192 Depression")) %>%
  mutate(Path = replace(Path, Path == "_nl_8", "Total Effect: Education \U2192 Mind-Body Medicine \U2192 Depression")) %>%
  add_significance(p.col = "p-value",
                   output.col = "Significance Level",
                   cutpoints = c(0, 0.001, 0.01, 0.05, 1),
                   symbols = c("***", "**", "*", " ")) %>% 
  ...
  knitr::kable(digits = 3, 
               ...
               escape = F, 
               col.names = c("Path", "\\textit{b}", "se", "\\textit{p}-value"," ", "Lower CI", "Upper CI"),
               caption="GSEM Indirect and Total Effects: SES, Mind-Body Medicines, and Depression") %>%
  kable_classic(html_font = "CMU Serif") %>%
  kable_styling(latex_options = c("striped")) %>% 
  footnote(general = c("\\\\textit{b} = unstandardized coefficient"),
           symbol = c("$p<.05.$", "$p<.01.$", "$p<.001.$"),
...
# Drop last column 
manipulativedepression <- slice(manipulativedepression, 1:(n() - 14))
```
