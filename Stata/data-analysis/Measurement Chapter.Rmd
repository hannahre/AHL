...
# Introduction 
*Intro: Why should CAMS fit together? Why should we care? What are the advantages of analyzing this – pros and cons*
...
```{r, echo = FALSE, results = 'hide'}
# This code chunk creates a crosstab for prayer and spiritual healing
...
freqout.is1 <- freqout[seq(2, 28, 2),]
...
In this section, I will present factor analysis results. First, I will present results from the exploratory factor analysis. Second, I will present results from the confirmatory factor analysis conducted on the solution found in the exploratory factor analysis. 
...
```
The factorability of the 14 CAM items was examined. First, it was observed that all items correlated with at least one other item with a correlation of .3, suggesting factorability. Next, the Kaiser-Meyer-Olkin measure of sampling adequacy was `r kmo`, above the recommended value of at least .6. Bartlett's test of sphericity was significant ($\chi$^2^(`r bartlett$df`) = `r bart.chisq`, *p* <.01) indicating that the correlations between variables are higher than would be expected by chance. There is correlation adequacy. Finally, (Report communalities - which indicate that items share common variance with other items).    
...
### Results
```{r, echo = FALSE}
# This code chunk runs the CFA on Ayers and Kronenfelds model
# Factor 1 (Mind-Body medicine): Meditation, Guided imagery, relaxation, deep breathing 
# Factor 2 (Alternative Medical Systems): Ayurveda, Folk medicine, Naturopathy, Homeopathy,Herbs, vitamins, Biofeedback 
# Factor 3 (Prayer): 
# Factor  (Manipulative treatments): acupuncture, chiropractic, massage 

# Prayer or spiritual healing will not be included because there are not enough items for measure that latent variable 

# Subset acams to remove prayer and spiritual healing 


# 08/06/21 code below is from the cfa for the efa model. adjust for ayers and kronenfeld. 
# Assigning items to factors 
#ayers.kronenfeld.model <- 'f1 =~ aPraySpirit + aHypnosis + aImageryTech + aRelaxMeditate + aEnergyHeal
...
The cluster rotation factor loadings for the 13 CAM types are shown in Table \ref{tab:efa-loadings-table}. Factor 1, 
